...
const LABELS: Record<string, string> = {
  exif_full: "analysisProgress.exif", phash: "analysisProgress.fingerprints", embed_image: "analysisProgress.embeddings",
  ai_tags: "analysisProgress.aiTags", import_tags: "analysisProgress.imported", faces: "analysisProgress.faces",
};
...
function state(a: AnalyzerStatusDto, paused: boolean, t: TFunction): { label: string; tone: "muted" | "accent" | "amber" | "red" | "green" } {
  const c = a.counts;
  const left = c.pending + c.running;
  if (!a.enabled) return { label: t("analysisProgress.off"), tone: "muted" };
  if (a.backoffUntil) return { label: t("analysisProgress.waiting"), tone: "amber" };
  if (paused && left > 0) return { label: t("analysisProgress.paused"), tone: "muted" };
  ...
  if (left > 0 && c.failed > c.done && c.failed >= 20) return { label: t("analysisProgress.failing"), tone: "red" };
  if (left > 0) return { label: t("analysisProgress.working"), tone: "accent" };
  if (c.failed > 0) return { label: t("analysisProgress.retry"), tone: "red" };
  return { label: t("analysisProgress.done"), tone: "green" };
}
...
function formatEta(seconds: number, t: TFunction): string {
  if (seconds < 60) return t("analysisProgress.underMinute");
  if (seconds < 3600) return t("analysisProgress.aboutMinutes", { count: Math.round(seconds / 60) });
  return t("analysisProgress.aboutHours", { value: (seconds / 3600).toFixed(1) });
}
...
export default function AnalysisProgress({ only, onStatus, pollMs = 3000, hideWhenDone = false }: AnalysisProgressProps) {
  ...
  if (!status) return hideWhenDone ? null : <p className="text-sm text-muted">{t("common.loading")}</p>;
  const rows = status.analyzers.filter((a) => !only || only.includes(a.key));
  if (rows.length === 0) return hideWhenDone ? null : <p className="text-sm text-muted">{t("analysisProgress.nothing")}</p>;
  if (hideWhenDone && rows.every((a) => a.counts.pending + a.counts.running + a.counts.failed === 0)) return null;

  return (
    <div className="flex flex-col gap-3">
      {status.paused && <p className="text-xs text-muted">{t("analysisProgress.scanPaused")}</p>}
      {rows.map((a) => {
        const n = total(a);
        ...
        const s = state(a, status.paused, t);
        const left = a.counts.pending + a.counts.running;
        return (
          <div key={a.key} className="flex flex-col gap-1">
            <div className="flex flex-wrap items-baseline justify-between gap-2 text-sm">
              <span className="text-ink">
                {LABELS[a.key] ? t(LABELS[a.key]) : a.key} <span className="text-xs text-faint">{a.version}</span>
              </span>
              <span className="flex items-center gap-2 text-xs tabular-nums text-muted">
                {finished.toLocaleString()} / {n.toLocaleString()} · {pct}%
                {s.tone === "accent" && eta[a.key] !== undefined && <span>· {t("analysisProgress.left", { eta: formatEta(eta[a.key], t) })}</span>}
                {a.counts.failed > 0 && <span className="text-red-600">· {t("analysisProgress.failed", { count: a.counts.failed })}</span>}
                <span className={`rounded-full px-2 py-0.5 font-medium ${toneClass[s.tone]}`}>{s.label}</span>
              </span>
            </div>
            <div className="h-2 w-full overflow-hidden rounded-full bg-chip" role="progressbar" aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100} aria-label={LABELS[a.key] ? t(LABELS[a.key]) : a.key}>
              <div
            ...
            {!a.enabled && left > 0 && a.key === "faces" && (
              <p className="text-xs text-muted">{t("analysisProgress.peopleOff")}</p>
            )}
            {!a.enabled && a.key === "embed_image" && <p className="text-xs text-muted">{t("analysisProgress.aiOff")}</p>}
            {a.counts.failed > 0 && (
              <p className="text-xs text-muted">
                {t("analysisProgress.failed", { count: a.counts.failed })}
                {a.lastError && (
                  <>
                    {" "}- {t("analysisProgress.lastError")} <code className="text-ink">{a.lastError}</code>
                  </>
                )}
